...
<html>
    ...
    <body>
        <form action="update-withdr.jsp" method="post">
            ...
            <label for="deposite">Withdrawal Amount:</label><br>
    ...
    function updateTotal() {
        ...
        let withdrawalAmount = document.getElementById("deposite").value;

        // Convert the withdrawal amount to a number
        withdrawalAmount = parseFloat(withdrawalAmount);

        // If the withdrawal amount is a valid number and non-negative
        if (!isNaN(withdrawalAmount) && withdrawalAmount >= 0) {
            // Check if the withdrawal amount exceeds the available balance
            if (withdrawalAmount > initialTotalAmount) {
                // Show message if the withdrawal amount exceeds balance
                alert("Insufficient Balance");
                // Reset total amount display to the initial balance
                 document.getElementById("deposite").value = '';
                document.getElementById("totalamt").value = initialTotalAmount.toFixed(2);
            } else {
                // Update total amount if withdrawal is valid
                let totalAmount = initialTotalAmount - withdrawalAmount;
                // Display the updated total amount in the 'totalamt' field
                document.getElementById("totalamt").value = totalAmount.toFixed(2);
            }
        } else {
            // If the withdrawal amount is invalid, show error message
            alert("Invalid Withdrawal amount. Please enter a valid, non-negative number.");
            // Reset total amount display to the initial balance
            document.getElementById("totalamt").value = initialTotalAmount.toFixed(2);
        }
    }
